...
RUN wget https://codeload.github.com/PrestaShop/PrestaShop/zip/1.7.6.9 -O /opt/bitnami/prestashop/1.7.6.9.zip
...
RUN wget https://raw.githubusercontent.com/SmartGuyy/docker-prestashop/master/composer.json -P /opt/bitnami/prestashop/
RUN composer install -d /opt/bitnami/prestashop/
RUN php -d date.timezone=UTC ./opt/bitnami/prestashop/vendor/bin/phpunit -c /opt/bitnami/prestashop/tests/Unit/phpunit.xml --log-junit /opt/bitnami/prestashop/results.xml | true
